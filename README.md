...
- BLEデバイス登録UI
- 自動再接続 (登録済みデバイスにNVS保存→電源再投入後も自動接続)
...
**2回目以降 (自動接続):**
- 電源を入れると自動でスキャン→接続 (約3秒後)
- Dashboard画面に "Auto-connecting..." と表示される
- 接続先はESP32のNVSに保存されるため電源OFFでも保持
- 登録を解除するにはRegister画面で **BtnB** → `forgetDevice()` で解除
