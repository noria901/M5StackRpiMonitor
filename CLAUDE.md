...
[RPi] -- BLE GATT Server --> [M5Stack Core] Display
  |
  +-- Web UI (Flask) for M5Stack device management
```

### Components
1. **m5stack-firmware/** - M5Stack Core用 Arduino/PlatformIO ファームウェア
2. **rpi-daemon/** - RPi側 BLE常駐デーモン (Python)
3. **rpi-web/** - RPi側 M5Stack登録管理 Web UI (Flask)

## Tech Stack
- **M5Stack**: C++ / Arduino framework / PlatformIO
- **RPi Daemon**: Python 3 / bleak or bluez (D-Bus)
- **RPi Web UI**: Python 3 / Flask / HTML+CSS+JS
